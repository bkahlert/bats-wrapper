...
fix_tmpdir_permissions() {
  ...
  [ -e "$tmp_dir" ] || mkdir -p "$tmp_dir" || logr error "'$tmp_dir' could not be created"
  [ -d "$tmp_dir" ] || return 0
...
fix_home_permissions() {
  ...
  mkdir -p "$home" || logr error "'$home' could not be created"
  chown -R "$user" "$home"
  chmod -R u+rw "$home"
}
...
#   1 - name of the user
#   2 - name of the group
fix_docker_sock_permissions() {
  local -r user="${1:?user missing}"
  local -r group="${2:?group missing}"
  local -r docker_sock="/var/run/docker.sock"

  [ -S "$docker_sock" ] || return 0

  chown -R "$user:$group" /var/run/docker.sock
}
