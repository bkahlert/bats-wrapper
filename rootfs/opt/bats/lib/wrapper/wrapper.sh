...
load_lib() {
  local short_name=${1:?}
  local file="/opt/bats/lib/$short_name/load.bash"
  [ -f "$file" ] || die '%s: %s does not exist\n' 'wrapper.sh' "$file" >&2
...
#       will generate foo_status=0 foo_output="bar" foo_lines=("bar") foo_workdir="$TMPDIR/..."
different_run() {
  local prefix=${1?prefix missing} command=${2?command missing} dir copy
  dir=$(pwd)
  copy="$(mktemp "${TMPDIR%/}/assert-alias-XXXXXX")"
  [ ! -e "$copy" ] || rm "$copy"
  cp -R "$dir" "$copy"
...
# TODO fix bats plugin using --batsw:
